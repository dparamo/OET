<template>
    <div class="container">
        <div class="row">
            <div class="col-md-6"><button class="btn btn-success" data-toggle="modal" data-target="#staticBackdrop">Nuevo vehiculo</button></div>
            <div class="col-md-6">
                <div class="input-group mb-3">
                    <div class="input-group-prepend">
                        <span class="input-group-text" id="inputGroup-sizing-default">Buscar</span>
                    </div>
                    <input type="text" placeholder="Placa" class="form-control" name="search" v-model="busqueda" @keyup.enter="buscar()"></div>
                </div>
        </div>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" v-model="dato.vehi_placa" required>
                            </div>
                            <div class="form-group col-md-6">
                                <label for="inputPassword4">Color</label>
                                <input type="text" class="form-control" v-model="dato.vehi_color" required> 
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="inputEmail4">Marca</label>
                                <input type="text" class="form-control" v-model="dato.vehi_marca" required>
                            </div>
                            <div class="form-group col-md-6">
                        ...
                        <div class="row">
                            <div class="form-group col-md-6">
                                ...
                                <select class="form-control" v-model="dato.id_propietario" required>
                                    <option v-for="option in propietarios" v-bind:key="option.id" v-bind:value="option.id">{{option.prop_fnombre}}</option>
                                </select>
                            </div>
                            <div class="form-group col-md-6">
                                <label for="inputPassword4">Conductor</label>
                                <select class="form-control" v-model="dato.id_conductor" required>
                                     <option v-for="option in conductores" v-bind:key="option.id" v-bind:value="option.id">{{option.prop_fnombre}}</option>
                                </select>
                            </div>
                        </div>
                    </div>
...
<script>
    export default {
        ...
        methods:{
            ...
            this.dato = {id:'',vehi_placa:'',vehi_color:'',vehi_marca:'',vehi_tipo_vehiculo:'',id_propietario:'',id_conductor:''}
            axios.post('/vehiculo', datoNueva)
                .then((res) =>{
            ...
            eliminar(item ,index){
                const confirmacion = confirm(`desea eliminar el vehiculo: ${item.prop_fnombre}`);
                if (confirmacion) {
                    axios.delete(`/vehiculo/${item.id}`)
                    .then(()=>{
            ...
            editar(dato){
                ...
                const params = {vehi_placa: dato.vehi_placa,vehi_color: dato.vehi_color,vehi_marca:dato.vehi_marca,vehi_tipo_vehiculo:dato.vehi_tipo_vehiculo,id_propietario:dato.id_propietario,
                id_conductor:dato.id_conductor}
                axios.put(`/vehiculo/${dato.id}`, params)
                    .then(res=>{
                        axios.get('/vehiculo')
                        .then(res=>{
                            this.datos = res.data.vehiculo.data
